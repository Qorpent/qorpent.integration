...
namespace Qorpent.Integration.MongoDB.Wiki {
    /// <summary>
    ///     Реализация персистера Wiki на Mongo
    /// </summary>
    public class MongoDbWikiPersister : MongoDbBasedServiceBase, IWikiPersister {
        ...
        /// </summary>
        protected MongoDbWikiSerializer Serializer { get; set; }

        /// <summary>
        ///     Создание персистера
        /// </summary>
        public MongoDbWikiPersister() {
            Serializer = new MongoDbWikiSerializer();
        }
        ...
        public IEnumerable<WikiPage> FindPages(string search) {
            var queryJson = "{$or : [{_id : {$regex : '(?ix)_REGEX_'}}, {code : {$regex : '(?ix)_REGEX_'}},{title:{$regex: '(?ix)_REGEX_'}},{owner:{$regex: '(?ix)_REGEX_'}},{editor:{$regex: '(?ix)_REGEX_'}}]},".Replace("_REGEX_", search);
        ...
        private MongoDbWikiVersionCreateResult PushHistory(WikiPage wikiPage, string comment) {
            ...
            if (!writeResult.Ok) {
                return new MongoDbWikiVersionCreateResult(false, version, published, writeResult.ErrorMessage);
            }

            return new MongoDbWikiVersionCreateResult(true, version, published, "Version created to with comment: '" + comment + "'");
        }
        ...
        public object CreateVersion(string code, string comment) {
            ...
            if (current == null) {
                return new MongoDbWikiVersionCreateResult(false, null, DateTime.MinValue, "There is no page to clone");
            }
        ...
        public object RestoreVersion(string code, string version) {
            ...
            if (restored == null) {
                return new MongoDbWikiVersionCreateResult(false, null, DateTime.MinValue, "There is no page to restore");
            }

            var created = PushHistory(restored, "Restore version: " + version);
            Save(restored);

            return new MongoDbWikiVersionCreateResult(true, created.Version, created.Published, "The page was restored successful");
        }
	    ...
	    public void ChangeCode(string fromcode, string tocode) {
		    var page = Get(fromcode).FirstOrDefault();
		    if (null != page) {
				page.Code = tocode;
				Save(page);
			    Collection.Remove(
				    new QueryDocument(
					    BsonDocument.Parse("{_id : '" + fromcode + "'}")
					    )
