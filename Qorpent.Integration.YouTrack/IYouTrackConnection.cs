...
namespace Qorpent.Integration.YouTrack {
	/// <summary>
	/// 	Интерфейс соединения с сервером YouTrack, выполняет вход, выход из сессии
	/// </summary>
	public interface IYouTrackConnection : IDisposable {
		/// <summary>
		/// 	Выполняет запрос к серверу YouTrack
		/// </summary>
		/// <param name="request"> Запрос REST API YouTrack </param>
		/// <returns> Отклик REST API YouTrack </returns>
		...
		/// </summary>
		/// <param name="throwError"> True (по умолчанию) - выбрасывает исключение если соединение не работает </param>
